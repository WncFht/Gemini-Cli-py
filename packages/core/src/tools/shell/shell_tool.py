"""
This file is refactored from packages/core_ts/src/tools/shell.ts.
"""
...
from ..common import ToolExecuteConfirmationDetails
...
class ShellTool(BaseTool[ShellToolParams, ToolResult]):
    ...
    def validate_tool_params(self, params: ShellToolParams) -> str | None:
        ...
        if params.directory:
            p = Path(params.directory)
            if p.is_absolute():
                return "Directory must be relative."
            if not is_within_root(
                self.config.get_target_dir() / p, self.config.get_target_dir()
            ):
    ...
    ) -> ToolExecuteConfirmationDetails | bool:
