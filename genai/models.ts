...
import * as types from './types.js';
  ...
  export class Models extends BaseModule {
    ...
    /**
     * 【内部核心】生成内容的核心方法（非流式）。
     * 无论是生成普通文本、还是结构化的 JSON，最终都会调用此方法。
     * 它负责将用户提供的参数转换为符合 API 规范的请求体，并发送 HTTP 请求。
     *
     * @param params - 包含模型、内容和配置的生成参数。
     *   - 如果要生成JSON，`params.config` 中会包含:
     *     - `responseMimeType: 'application/json'`
     *     - `responseSchema: <一个JSON Schema对象>`
    ...
    private async generateContentInternal(
      ...
      if (this.apiClient.isVertexAI()) {
        // 1. 参数转换：将上层传入的、易于使用的JS对象 (params)，
        //    转换为符合 Vertex AI API 规范的底层请求体 (body)。
        //    我们设置的 `responseMimeType` 和 `responseSchema` 会在这里被处理和包含进去。
        ...
        // 2. API 调用：通过 apiClient 发送一个 HTTP POST 请求。
        //    请求体 `body` 被序列化为 JSON 字符串。
        //    Gemini 服务器端会解析这个请求体，识别到 JSON 生成的"契约"。
        ...
        // 3. 响应处理：接收到 API 的响应后，同样通过转换器将其从底层格式
        //    转换回高层、易于使用的 `GenerateContentResponse` 对象。
        ...
        // 同上，但这是针对 MLDev (Gemini API) 的参数转换逻辑。
    ...
    /**
     * 【内部核心】生成内容的流式版本。
     * 它请求 API 以 Server-Sent Events (SSE) 的形式返回数据。
     * 重要的是，它流式传输的不是纯文本，而是结构化的 JSON 块 (chunks)。
     * 每个块都是一个部分的 `GenerateContentResponse`。
     * 这就是为什么即使在流式传输中，SDK也能立刻判断出模型是在生成文本、函数调用还是思考过程。
     *
     * @param params - 与非流式版本相同的参数。
     * @returns 一个 `AsyncGenerator`，它会不断 yield 出部分的 `GenerateContentResponse`。
     */
    private async generateContentStreamInternal(
      ...
      if (this.apiClient.isVertexAI()) {
        ...
        return response.then(async function* (
          ...
          // 循环处理从服务器发来的每一个数据块 (chunk)。
          for await (const chunk of apiResponse) {
            // 将每个JSON块转换为标准响应对象并 yield 出去。
        ...
        // 同上，但这是针对 MLDev (Gemini API) 的流式逻辑。
        const body = converters.generateContentParametersToMldev(
          this.apiClient,
          params,
        );
